...
import 'package:almanac/features/today/presentation/widgets/explore_links.dart';
import 'package:almanac/features/today/presentation/widgets/moon_disc.dart';
import 'package:almanac/features/today/presentation/widgets/sky_hero.dart';
...
void main() {
  ...
  group('the shape of the page', () {
    ...
    testWidgets('links onward to the sections that exist, and no others', (
      tester,
    ) async {
      await openToday(tester, overrides: environmentOverrides());

      expect(find.text('Elsewhere'), findsOneWidget);
      for (final section in ['Wellbeing', 'Rhythms', 'Nature', 'Food']) {
        // Once in the nav bar, once as a link.
        expect(
          find.text(section),
          findsNWidgets(2),
          reason: '$section should appear as a link and in the nav bar',
        );
      }
    });

    testWidgets('a link goes to the real screen', (tester) async {
      await openToday(tester, overrides: environmentOverrides());

      await tester.tap(
        find.descendant(
          of: find.byType(ExploreLinks),
          matching: find.text('Nature'),
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('navigation is unchanged: still five destinations', (
      tester,
    ) async {
      await openToday(tester, overrides: environmentOverrides());
    ...
    testWidgets('settings is still one tap away', (tester) async {
      await openToday(tester, overrides: environmentOverrides());
